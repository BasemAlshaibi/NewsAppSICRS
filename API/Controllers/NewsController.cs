    ...
    public class NewsController : BaseApiController
    {
     ...
     //  [Authorize]
      ...
      [ServiceFilter(typeof(NoOfReadIncrement))]
       ...
       // [Authorize]
        [HttpPost]
        [RequestSizeLimit(2 * 1024 * 1024)]// Up to two megabytes are allowed to be uploaded at a time
        ...
        {
//// The following code will have no effect because the images come one at a time and not an array of images 
            if (images.Length > 10)
            {
...
// It means that there is already existing news to which we want to upload the image
            if (newsId.HasValue)
            {
          ...
          // If the image is uploaded during editing and this image is the first image, here we make it the main one
...
// Here, delete an image from the data base and from the storage space as well
                var spec = new NewsSpecification(newsId.Value);
            ...
            else
            {
             ...
             // It means that the image has not yet been saved in the data base, so it is enough to delete it from storage
